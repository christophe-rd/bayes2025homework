...
```{r}
## read empirical data
fastswim <- read.table(here("data","fastswim.dta"), sep = "", header = TRUE, na.strings = ".")
```
```{r}
### simulate data
...
#N = 565
Nspp = 16
#species <- sample(species_codes, size = N, replace = TRUE)
#individ <- c(1:N)

individs_per_spp <- round(runif(Nspp, 20, 30)) ### determines number of individuals per species
species <- rep(1:Nspp, individs_per_spp) ## creates vector of species code for each individual in sp. group
N <- length(species) ### length of species vector (# of individuals)

individ <- rep(NA, N)

### loop to create a sequence of individual per species, 1 through individs_per_spp
for (sp in 1:Nspp){
  individ[species==sp] <- 1:(individs_per_spp[sp])
}
...
## I think, given the above plots, I believe we've attained a fairly good fit. Yet, our residuals are fairly large, meaning we have some unaccounted for variance. Given that we have several additional measured variables, we could go back and consider whether other predictors may be of importance. However, we have learned that mass (alone) IS a decent predictor of swimming speed. The additional variance is likely due to a number of different morphological and/or physiological variables.
